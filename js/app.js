const allCategoryFetch = async () => {
    ...
    data.data.forEach(element => {
      ...
      categoryListButton.innerHTML = `<button id="${element.category_id}" onClick=" itemFetchWithId('${element.category_id}')" class="btn w-[80px] defaultButton">${element.category}</button>`;
...
async function itemFetchWithId(params = '1000') {
    loadingOption(true)
...
function loadingOption(isLoading) {
  ...
  function sortThis(data) {
    ...
    function compare(a, b){
      ...
      // console.log(parseFloat(b.others.views));
